...
import { ALPHA, BETA } from '../lib/constants.jsx'
...
export function useSwarm() {
	...
	const syncRemoteBestRoute = useCallback((payload) => {
		...
		setBestRoute((current) => {
			const currentDistance = Number.isFinite(payload.bestDistance)
				? payload.bestDistance
				: Number.POSITIVE_INFINITY
			return current.length > 1 && currentDistance >= stats.bestDistance ? current : payload.bestRoute
		})
	...
	const reset = useCallback(() => {
		const resetEdges = cloneEdges(INITIAL_EDGES)
		setEdges(resetEdges)
		setBestRoute([])
		setStats((current) => ({
			...current,
		...
		workerRef.current?.postMessage({ type: 'RESET', payload: { edges: resetEdges } })
		setIsRunning(false)
	}, [])
	...
	return useMemo(() => ({
		...
		setParameter,
		toggleEdge,
	}), [bestRoute, edges, isRunning, nodes, params, reset, setParameter, start, stats, stop, toggleEdge])
}
